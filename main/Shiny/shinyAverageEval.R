...
longDf1 <- as.data.frame(read_parquet("PFfullDf.parquet"))
longDf2 <- as.data.frame(read_parquet("PFfullDf2.parquet"))
...
issueDf1 <- issueDf1 %>% select(label, bootInOut)
issueDf2 <- issueDf2 %>% select(label, bootInOut)
issueDf1$study <- "Study 1"
issueDf2$study <- "Study 2"
issueDf <- rbind(issueDf1,issueDf2)
issueDf$bootInOut <- abs(issueDf$bootInOut)
alldfsumm <- merge(alldfsumm,issueDf)
alldfsumm <- alldfsumm %>% arrange(bootInOut)
# Ordered <- alldfsumm[order(-alldfsumm$bootInOut), "label"]
# alldfsumm <- alldfsumm %>% mutate(label = factor(label, levels = Ordered, ordered = TRUE))
...
server <- function(input, output) {
  ...
  plot_data <- reactive({
    ...
    if(input$partyN2=="Split"){
      ...
      ggplot(dataset2, aes_string(x=paste0("reorder(label, -bootInOut)")
                                  , y="eval",group="partyN", fill="partyN")) +
